...
// --- SVG ICONS ---
// A collection of SVG paths for all the icons used in the app.
const ICONS = {
...
/**
 * A reusable, animated icon component.
...
/**
 * A reusable, animated button component for the player controls.
 * @param {function} onClick - The function to call on click.
 * @param {boolean} disabled - Whether the button is disabled.
 * @param {string} iconPath - The SVG path for the icon.
...
function App() {
    ...
    const API_KEY = "AIzaSyDOSbOYdwTnhb0hrJVing6Pm0b_jKT5Itg"; 
    ...
    // Effect to create, update, and destroy the player instance
    useEffect(() => {
    ...
    // Effect to handle the progress bar updates
    useEffect(() => {
        if (isPlaying) {
    ...
    // Effect to perform an initial search on load
    useEffect(() => {
        handleSearch();
    }, []);
    ...
    const onPlayerError = (event) => {
        ...
        playNext(); // Try to play the next song
    }
